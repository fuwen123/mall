    ...
    <div class="common-header-wrap">
      <mt-header :title="title"
        ...
        <mt-button slot="right"
                   @click="historyVisible=true">聊天记录</mt-button>
      </mt-header>
    </div>
    <div class="dstouch-chat-con"
         id="dstouch-chat-con">
      <div v-for="(item,index) in msg_list">

        <chat-message :item="getMessage(item,index,0)"
                      :t_id="t_id"
                      :memberInfo="memberInfo"
                      :userInfo="userInfo"></chat-message>
      </div>
    </div>
    <div class="dstouch-chat-bottom">
      <div class="chat-input-layout"> <span class="open-smile"><a class="iconfont"
             @click="open_smile">&#xe6cd;</a></span>
        <div class="input-box">
          <input type="text"
                 v-model="msg"
          ...
          <mt-button class="submit"
                     size="small"
                     type="danger"
                     @click="submit">发送</mt-button>
        </div>
      </div>
      <div class="chat-smile-layout"
           v-show="smile_show">
        <ul>
          <li v-for="(smile,index) in smilies_array"
              @click="addSmile(index)"><img :title="smile.text"
                 :alt="smile.text"
                 :src="getSmile(smile.image)"></li>
        </ul>
      </div>
    </div>

    <!--聊天记录-->
    <mt-popup v-model="historyVisible"
              position="right"
              class="common-popup-wrapper">
      <div class="common-header-wrap">
        <mt-header title="聊天记录"
                   class="common-header">
          <mt-button slot="left"
                     icon="back"
                     @click="historyVisible=false"></mt-button>
        </mt-header>
      </div>
      <div class="common-popup-content">
        <div v-infinite-scroll="loadMore"
          ...
          <div v-if="historyList && historyList.length">
            <chat-message v-for="(item,index) in historyList"
...
.chat-input-layout .open-smile {
  display: block;
  width: 1.5rem;
  height: 1.5rem;
  padding: 0.25rem;
}
.chat-input-layout .open-smile a {
  display: block;
  width: 1.5rem;
  height: 1.5rem;
  font-size: 1.5rem;
...
.chat-input-layout .input-box input[type="text"] {
...
.chat-input-layout .input-box .submit {
  position: absolute;
  z-index: 1;
  top: 0;
  right: 0;
...
.chat-smile-layout ul {
  font-size: 0;
}
.chat-smile-layout ul li {
