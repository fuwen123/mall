...
class Memberpoints extends MobileMember
{
    ...
    public function points_signin(){
        ...
        $condition_arr=array();
        $condition_arr['pl_memberid'] = $this->member_info['member_id'];
        $condition_arr['pl_addtime'] = array('between',[$start_time,$end_time]);
        $condition_arr['pl_stage'] = 'signin';
        ...
        $if_signin=$points_model->getPointsInfo(array('pl_memberid'=>$this->member_info['member_id'],'pl_stage'=>'signin','pl_addtime'=>array('between',[$today_time,$today_time+86399])));
    ...
    private function insertarr($stage)
    {
        ...
        switch ($stage) {
            case 'regist':
                $insertarr = lang('pl_stage_regist');
                break;
            case 'login':
                $insertarr = lang('pl_stage_login');
                break;
            case 'comments':
                $insertarr = lang('pl_stage_comments');
                break;
            case 'order':
                $insertarr = lang('pl_stage_order');
                break;
            case 'system':
               	$insertarr = lang('pl_stage_system');
               	break;
            case 'pointorder':
                $insertarr = lang('pl_stage_pointorder');
                break;
            case 'app':
               	$insertarr = lang('pl_stage_app');
               	break;
            case 'signin':
                $insertarr = lang('pl_stage_signin');
                break;
            case 'marketmanage':
                $insertarr = lang('pl_stage_marketmanage');
                break;
        }
        return $insertarr;
    }
    ...
    public function signin_add()
    {
        ...
        $if_signin_series=$points_model->getPointsInfo(array('pl_memberid'=>$this->member_info['member_id'],'pl_stage'=>'signin','pl_addtime'=>array('between',[$today_time-86400,$today_time-1])));
        $points_signin = intval(config('points_signin'));//签到对得积分数
        $points_signin_cycle=intval(config('points_signin_cycle'));
        $points_signin_reward=intval(config('points_signin_reward'));
        $edit_member=array(
        ...
        if($return){
            model('member')->editMember(array('member_id' => $this->member_info['member_id']), $edit_member);
        }
