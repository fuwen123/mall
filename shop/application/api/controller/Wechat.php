        ...
        if ($this->type == 'event') {
            ...
            if (isset($event['event']) && $event['event'] == 'subscribe') {
                ...
                if(!empty($mbusertoken)){
                    $ret_url='。系统已为您自动注册了一个账号，请<a href="'.H5_SITE_URL.'/member/index?key='.$mbusertoken['member_token'].'&username='.$mbusertoken['member_name'].'">点击修改信息</a>';
                }else{
                    $ret_url='';
                }
    ...
    *文本格式消息回复
    ...
    private function MsgTypeNews($reMsg){
            ...
            foreach ($reMsg as $v) {
                ...
                $newsData[$k]['Url'] = H5_SITE_URL . '/home/goodsdetail?goods_id='.$v['goods_id'];
                $k++;
            }
