...
class Article extends MobileMall
{
    ...
    public function article_list() {
        ...
        if ($ac_id > 0) {
            $prefix = 'api-article-list-';
            $result = rcache($ac_id,$prefix);
            if (empty($result)) {
                ...
                $result['article_list'] = $article_model->getArticleList($condition);
                $result['article_type_name'] = $this->article_type_name($ac_ids);
                wcache($ac_id, $result,$prefix, 3600);
            }
            ds_json_encode(10000, '',array('article_list' => $result['article_list'], 'article_type_name' => $result['article_type_name']));
        } else {
