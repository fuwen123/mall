            ...
            if ($store_info) {
                try {
                    ...
                    if (!$store_info['store_bill_time']) {
                        ...
                        $order_time = db('order')->where(array('store_id' => $store_id, 'order_state' => ORDER_STATE_SUCCESS, 'delay_time' => array('<', $temp_time), 'payment_code' => array('<>', 'offline'), 'ob_no' => 0))->order('add_time asc')->lock(true)->value('add_time');
                        //不允许退款且超过使用期限的、允许退款且超过可退款期限、允许退款并且全部完成退款的虚拟订单
                        $vr_order_time = db('vrorder')->where(array('store_id' => $store_id, 'order_state' => ORDER_STATE_SUCCESS, 'finnshed_time' => array('<', strtotime(date('Y-m-d 0:0:0')) - intval(config('store_bill_cycle')) * 86400), 'ob_no' => 0))->where('((vr_invalid_refund=0) || (vr_invalid_refund=1 AND vr_indate<' . $vr_order_refund_time . ') || (vr_invalid_refund=1 AND refund_state=2))')->order('add_time asc')->lock(true)->value('add_time');
                    ...
                    if ($ob_startdate) {
                        ...
                        if ($order_ids) {
                            ...
                            if ($order_ids) {
                                ...
                                //结算退还佣金，部分退款的
                                $refundreturn_sum_1 = db('refundreturn')->alias('refundreturn')->join('__ORDERGOODS__ ordergoods', 'refundreturn.order_goods_id = ordergoods.rec_id')->where(array('refundreturn.order_id' => array('in', $order_ids), 'refundreturn.refund_state' => 3, 'refundreturn.seller_state' => 2, 'refundreturn.order_goods_id' => array('>', 0)))->field('SUM(ROUND(refundreturn.refund_amount*ordergoods.commis_rate/100,2)) AS ob_commis_return_totals')->lock(true)->find();
                                $refundreturn_sum_2 = array();
                                $refundreturn_rec_ids_2 = db('refundreturn')->where(array('order_id' => array('in', $order_ids), 'refund_state' => 3, 'seller_state' => 2, 'order_goods_id' => 0))->lock(true)->column('order_id');
                                if ($refundreturn_rec_ids_2) {
                                    $refundreturn_sum_2 = db('ordergoods')->where(array('order_id' => array('in', $refundreturn_rec_ids_2)))->field('SUM(ROUND(goods_pay_price*commis_rate/100,2)) AS ob_commis_return_totals')->lock(true)->find();
                                }
