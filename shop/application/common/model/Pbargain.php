...
class Pbargain extends Model {
     ...
     * @return array 砍价列表
     */
    public function getBargainList($condition, $page = null, $order = 'bargain_id desc', $field = '*') {
        $res = db('pbargain')->field($field)->where($condition)->order($order)->paginate($page, false, ['query' => request()->param()]);
     ...
     * @return array 砍价列表
     */
    public function getOnlineBargainList($condition, $page = null, $order = 'bargain_id desc', $field = '*') {
        $condition['bargain_state'] = self::PINTUAN_STATE_NORMAL;
        $condition['bargain_endtime'] = array('>',TIMESTAMP);
        $bargain_list = $this->getBargainList($condition, $page, $order, $field);
        return $bargain_list;
    }
