...
var province_id,city_id,district_id,province_name,city_name,district_name,town_id,town_name;
//locationInitialize();
try{
    auto_location();
}catch (e) {
...
$('body').on('click', '.area-list p', function () {
  ...
  town_id = '';
  town_name = '';
  setCookies('town_id', city_name, 30 * 24 * 60 * 60 * 1000);
  setCookies('town_name', district_name, 30 * 24 * 60 * 60 * 1000);
  //ajaxDispatching();
  ajaxTown(district_id);
});
//选择区 镇
$('body').on('click', '.town-list p', function () {
  town_id = $(this).attr('data-id');
  town_name = $(this).text();

  delCookie('town_id');
  delCookie('town_name');
  setCookies('town_id', town_id, 30 * 24 * 60 * 60 * 1000);
  setCookies('town_name', town_name, 30 * 24 * 60 * 60 * 1000);
  ajaxDispatching();
  hideP();
...
function hideP(){
  ...
  if(typeof(select_area_callback)=='function'){
    ...
    select_area_callback(province_name ,city_name ,district_name,province_id,city_id,district_id,town_name,town_id);
  }
  undercover();
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if(district_count == 0){
        ...
        $('.town-list').empty().append(district_html);
        $('.area-list').hide();
        $('.town-list').show();
      }
  ...
  if(typeof(goods_id) != 'undefined' && region_id!= ''){
    $.ajax({
      ...
      success: function (data) {
        ...
        town_name = town_name || '';
        $('#address').text(province_name+city_name+district_name+town_name);
      }
    });
  }
}
...
function set_area_id(addComp){
  ...
  console.log(province_id,city_id,'set_id')
}
// 获取省id
