...
class Api extends Base
{
    ...
    public function getRegionIdByName()
    {
        $name = I('name');
        $city_id = I('city_id');
        $where['parent_id'] = $city_id;
        $where['name'] = $name;
        $arr = Db::name('region')->where($where)->find();
        if(!$arr){
            $arr['id'] = 0;
        }
        $this->ajaxReturn($arr);
    ...
    public function queryExpress()
    {
        ...
        if ($express_switch == 1) {
            ...
            $data['LogisticCode'] = trim(I('invoice_no'));
...
//        $ip = '183.147.30.238';//测试ip
    ...
    public function shop()
    {
        ...
        if ($shop_list) {
            ...
            $ak = tpCache('basic.bd_serverak'); //Sgg73Hgc2HizzMiL74TUj42o0j3vM5AL
            $url = 'http://api.map.baidu.com/routematrix/v2/driving?output=json&origins=' . implode('|', $origins) . '&destinations=' . implode('|', $destinations) . '&ak='.$ak;
