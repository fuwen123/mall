...
class Comment extends Base {
    ...
    public function commentHandle(){
        ...
        $row = false;
        $goods_ids = $row = M('comment')->where('comment_id', 'IN', $comment_ids)->whereOr('parent_id', 'IN', $comment_ids)->value('goods_id');
        if ($type == 'del') {
            //删除咨询
            $row = M('comment')->where('comment_id', 'IN', $comment_ids)->whereOr('parent_id', 'IN', $comment_ids)->delete();
            db('goods')->where('goods_id','IN',$goods_ids)->setDec('comment_count');

        }
        if ($type == 'show') {
//            $row = M('comment')->where('comment_id', 'IN', $comment_ids)->save(['is_show' => 1]);
            db('goods')->where('goods_id','IN',$goods_ids)->setInc('comment_count');
        }
        if ($type == 'hide') {
