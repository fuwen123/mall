...
//            }
    ...
    public function send_list(){
		...
		$count =  db('coupon_list')->where($condition)->count();
    	$Page = new Page($count,20);
        $show = $Page->show();
        $lists = M('coupon_list')->where($condition)->order('send_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        $this->assign('lists',$lists);
        $this->assign('pager',$Page);// 赋值分页输出
        $this->assign('page',$show);// 赋值分页输出   
        $this->assign('coupons',C('COUPON_TYPE'));
		if(!empty($lists)){
			...
			$users = db('users')->where('user_id in('.implode(',', $user_id_arr).')')->getField('user_id,nickname');
			$coupon_id_arr = get_arr_column($lists, 'cid');
			$coupon = db('coupon')->where('id in('.implode(',', $coupon_id_arr).')')->getField('id,name,money,use_end_time');
