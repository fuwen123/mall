...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $count = $Order->alias('o')->field('order_id')->join('__USERS__ u','u.user_id = o.user_id','left')->where($condition)->count();
        $Page  = new AjaxPage($count,20);
        $show = $Page->show();
        $orderList = $Order->alias('o')->field('o.*,nickname')->join('__USERS__ u','u.user_id = o.user_id','left')->where($condition)->limit($Page->firstRow,$Page->listRows)->order($sort_order)->select();
...
exit("商业用途必须购买正版,使用盗版将追究法律责任");
    ...
    public function ajaxdelivery(){
        ...
        $listRows = I('listRows') ? I('listRows') : 10;
    ...
    public function refund_order_info($order_id){
        ...
        if($order['pay_status'] == 3 && $order['orderGoods'][0]['goods']['is_virtual'] == 1){
            $this->error('查看虚拟订单详情',U("admin/Order/virtual_info",['order_id'=>$order_id]),1);
            exit;
        }
        $this->assign('order',$order);
        return $this->fetch();
    ...
    public function getOrderAction(){
        ...
        foreach ($action_log as $k => $v){
            if ($v['group'] == 0) {
				...
				$suppliers_name = M("suppliers")->where(['suppliers_id'=>$v['action_user']])->value('suppliers_name');
...
exit("商业用途必须购买正版,使用盗版将追究法律责任");
    ...
    public function delivery_excel()
    {
        if(IS_POST){
            ...
            $shipping_list = db('shipping')->field('s.shipping_name,s.shipping_code')->alias('ss')->join('__SHIPPING__ s','s.shipping_id = ss.shipping_id','left')->select();
            $this->assign('shipping_list',$shipping_list);
            $this->assign('order',$order);
...
exit("商业用途必须购买正版,使用盗版将追究法律责任");
    ...
    public function delReturnList(){
        ...
        M('return_goods')->whereIn('id',$returnListIds)->delete();
    ...
    public function refund_back(){
        ...
        if($order['pay_code'] == 'weixinH5' || $order['pay_code'] == 'weixin' || $order['pay_code'] == 'miniAppPay'  || $order['pay_code'] == 'appWeixinPay'
            ...
            if($order['pay_code'] == 'weixinH5' || $order['pay_code'] == 'weixin' || $order['pay_code'] == 'miniAppPay'  || $order['pay_code'] == 'appWeixinPay'){
                ...
                $payment_obj = new \newalipay();
                $refund_data = array('order_id' => $rec_goods['rec_id'], 'out_trade_no' => $order['order_sn'], 'refund_amount' => $order['order_amount'], 'refund_reason' => $data['admin_note'], 'type' => 2);
    ...
    public function add_return_goods()
   {
    ...
    public function order_action(){     
        ...
        if($action && $order_id){
			...
			$a = $orderLogic->orderProcessHandle($order_id,$action,array('note'=>I('note'),'admin_id'=>$this->admin_id));
             if($res !== false && $a !== false){
                 if ($action == 'remove') {
					 ...
					 if ($order && $order['deleted']) {
						$this->ajaxReturn(['status' => 1,'msg' => '操作成功，此为复合订单，已拆分','url' => U('Order/index')]);
					 }
				 }
    ...
    public function export_order()
    {
        ...
        } elseif (I('is_delivery_list')) {
			//发货单页面的打印
			I('consignee') ? $where['o.consignee'] = trim(I('consignee')) : false;
			I('nickname') ? $where['o.nickname'] = trim(I('nickname')) : false;
			I('order_sn') != '' ? $where['o.order_sn'] = trim(I('order_sn')) : false;
			$shipping_status = I('shipping_status');
			$where['o.shipping_status'] = empty($shipping_status) ? array('neq',1) : $shipping_status;
			$where['o.order_status'] = array('in','1,2,4');
			$where['o.prom_type'] = ['neq',5];
			$where['o.shop_id'] = 0;
		} elseif (I('is_refund_order_list')) {
			//退款单页面的打印
			I('consignee') ? $where['o.consignee'] = trim(I('consignee')) : false;
			I('order_sn') != '' ? $where['o.order_sn'] = trim(I('order_sn')) : false;
			I('mobile') != '' ? $where['o.mobile'] = trim(I('mobile')) : false;
			$prom_type = I('export_prom_type');
			if($prom_type){
				$where['o.prom_type'] = $prom_type;
			}
			$where['o.shipping_status'] = 0;
			$where['o.order_status'] = 3;
			$where['o.pay_status'] = array('gt',0);
		} else {
			...
			if(I('pay_code')){
				switch (I('pay_code')){
					...
					case '其他方式':
						...
						break;
					default:
						$where['o.pay_code'] = I('pay_code');
						break;
				}
			}
			I('pay_status') != '' ? $where['o.pay_status'] = I('pay_status') : false;
            $shipping_status = I('shipping_status');
			if($where['o.order_status'] == 3){
				$where['o.pay_status'] = ['gt',0];
				$where['o.shipping_status'] = 0;
...
//                  $strGoods .= "<br />";
	...
	public function getSupplierOrderIds(){
		...
		$orderIds = Db::name('order_goods')
			...
			->getField('og.order_id,og.order_id');
		if ($orderIds) {
    ...
    public function ajaxSupplierOrderList(){
		...
		$condition['o.suppliers_id'] = ['neq', 0];
		$condition['o.deleted'] = 0;
		I('order_statis_id', '') && $condition['o.order_statis_id'] = I('order_statis_id');
        $keyType = I("key_type");
        $keywords = I('keywords','','trim');
		if ($keywords) {
			switch ($keyType) {
        ...
        if(I('pay_code')){
            switch (I('pay_code')){
                case '余额支付':
                    $condition['o.pay_name'] = I('pay_code');
                    break;
                case '积分兑换':
                    $condition['o.pay_name'] = I('pay_code');
                    break;
                ...
                case '其他方式':
                    ...
                    break;
                default:
                    $condition['o.pay_code'] = I('pay_code');
                    break;
            }
        }
	...
	public function supplierRefundList(){
        ...
        I('consignee') ? $condition['o.consignee'] = ['like', '%'.trim(I('consignee')).'%'] : false;
        I('order_sn') != '' ? $condition['o.order_sn'] = ['like', '%'.trim(I('order_sn')).'%'] : false;
        I('mobile') != '' ? $condition['o.mobile'] = ['like', '%'.trim(I('mobile')).'%'] : false;
		I('suppliers_name') != '' ? $condition['s.suppliers_name'] = ['like', '%'.trim(I('suppliers_name')).'%'] : false;
        $prom_type = input('prom_type');
        if($prom_type){
        ...
        $condition['o.pay_status'] = array('gt',0);
        $count = M('order')
			->alias('o')
			->join('suppliers s', 's.suppliers_id=o.suppliers_id', 'left')
			->where($condition)
    ...
    public function ajaxSupplierReturnList(){
        ...
        // 搜索条件
        $order_sn =  trim(I('order_sn'));
		$suppliersName =  trim(I('suppliers_name'), '');
        $order_by = I('order_by','') ? I('order_by') : 'id';
        $sort_order = I('sort_order') ? I('sort_order') : 'desc';
        $status =  I('status');
        $where = [];
