...
//        $user = D('users')->where(array('user_id' => $uid))->find();
    ...
    public function transfer()
    {
        ...
        foreach ($withdrawals as $val) {
			...
			$money = $user['distribut_money'] - $val['distribut_withdrawals_money']; //可提现佣金
                       ...
                       //type 支出类型0用户提现,1订单退款,2其他,3注册,4邀请,5分享,6评论
                        ...
                        if ($res && $res['status'] != 0) {
                            if($val['type'] == 1){
                                ...
                                accountLog($val['user_id'], ($val['money'] * -1), 0, "平台处理用户提现申请",0,0,'',false,true);
                            }
                            M('withdrawals')->where(array('id' => $val['id']))->save(array('status' => 2, 'pay_time' => time(), 'pay_code' => $res['send_listid']));
                            expenseLog($rdata);
                            // 提现通知
...
exit("商业用途必须购买正版,使用盗版将追究法律责任");
    ...
    public function check_first_leader()
    {
        ...
        if (IS_POST && $mobile && $user_id) {
            ...
            $first_leader = $Users::get(['mobile' => $mobile, 'is_lock' => 0]);
            $users = db('users')->where(['user_id' => $user_id])->find();
    ...
    public function momentsList()
    {
        ...
        $Page = new Page($count);
        $show = $Page->show();
        $list = $moments->alias('m')->limit($Page->firstRow, $Page->listRows)
    ...
    {
        $find = Db::name('moments')->alias('m')->join('__USERS__ u', 'u.user_id = m.user_id', 'LEFT')->field("m.*,nickname,mobile")->find($id);
    ...
    public function commentList()
    {
        ...
        $Page = new Page($count);
        $show = $Page->show();
        $list = $comment->alias('m')->limit($Page->firstRow, $Page->listRows)
            ->join('__USERS__ u', 'u.user_id = m.user_id', 'LEFT')
            ->field("m.*,nickname,mobile")
            ->order('m.is_delete asc ,m.status asc,m.add_time asc')->select();
    ...
    {
        $id = I('post.comment_id/d', 0);
        $find = Db::name('moments_comment')->alias('m')->join('__USERS__ u', 'u.user_id = m.user_id', 'LEFT')->field("m.*,nickname,mobile")->find($id);
        $this->assign('vo', $find);
        return $this->fetch();
    }
