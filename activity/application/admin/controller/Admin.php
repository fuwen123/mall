...
class Admin extends Base {
	...
	public function delAdmins(){
		...
		$this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/Shipping/index")]);
	}
	...
	public function delRoles(){
		...
		empty($ids) && $this->ajaxReturn(['status' => -1,'msg' =>"非法操作！",'data'  =>'']);
		$rolesIds = rtrim($ids);
		$admin = D('admin')->whereIn('role_id',$rolesIds)->find();
		if ($admin) {
			$this->ajaxReturn(['status' => -1,'msg' => '请先清空所属该角色的管理员','url'=>U("Admin/Admin/role")]);
		} else {
			M('admin_role')->whereIn('role_id',$rolesIds)->delete();
			$this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/Admin/role")]);
		}
	}
