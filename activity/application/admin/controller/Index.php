...
//    	$today = strtotime("-1 day");
        ...
        $count['stock'] = M('spec_goods_price')->where("store_count<$warning_storage")->count();
...
//            if ($table == 'prom_order') {
//                $promOrderModel = new \app\common\model\PromOrder();
//                $data = Db::name('prom_order')->where(['id'=>$id_value])->field('start_time,end_time')->find();
//                $type = I('type');
//                if (in_array($type,['0','1'])) {
//                    $type = ['in', [0, 1]];
//                    $other_activity = '满额打折和满额优惠金额';
//                } else {
//                    $type = ['in', [2, 3]];
//                    $other_activity = '满额送积分和满额送优惠券';
//                }
//                $orderWhere = [
//                    'id' => ['<>',$id_value],
//                    'type'=> $type,
...
//                ];
//                $count = $promOrderModel->where($orderWhere)->count();
//                if ($count >= 1) {
//                    $this->ajaxReturn(['status' => -1,'msg'=>$other_activity.'只能同时开启一个,或者同种类型活动已正在开启']);
//                }
//            }
