...
class Base extends Controller {
    ...
    function __construct() 
    {
        ...
        if(1 == IS_SAAS){
            if(!session('notices')){
                session('notices',(new \app\common\logic\Notice)->show());
            }
            $this->assign('notices',session('notices'));
        }
   }
    ...
    public function _initialize()
    {
        ...
        //过滤不需要登陆的行为 
        if (!in_array(ACTION_NAME, array('login', 'vertify', 'addTask','toSystemLoginTask', 'DeliveryTimeout'))) {
