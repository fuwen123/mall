...
class Article extends Base {
    ...
    public function delList(){
        ...
        $listIds = rtrim($ids);
        M('article')->whereIn('article_id',$listIds)->delete();
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/Article/articleList")]);
    }
    ...
    public function delLinks(){
        ...
        empty($ids) && $this->ajaxReturn(['status' => -1,'msg' =>"非法操作！",'data'  =>'']);
        $link_ids = rtrim($ids);
        M('friend_link')->whereIn('link_id',$link_ids)->delete();
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/article/linkList")]);
    }
