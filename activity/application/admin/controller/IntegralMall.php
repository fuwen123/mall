...
class IntegralMall extends Base
{
    ...
    public function handle()
    {
        ...
        $param['is_use_integral'] = ($param['is_use_integral'] == '0') ? '0' : '1';
...
//    }
    ...
    {
        //修改密码
        $enOldPwd = encrypt(input('pass'));
        $admin = M('admin')->where('admin_id' , session('admin_id'))->find();
        if(!$admin || $admin['password'] != $enOldPwd){
            $this->ajaxReturn(['msg' => '密码不正确！','status' => 0, 'result' => '']);
        }
        if($admin){
            //避免更新数据需要长时间，这里先设置不限制php超时时间
            ini_set("max_execution_time",0);
            $user = Users::all(['pay_points'=>['>',0]]);
            if($user){
                foreach ($user as $k=>$v)
                {
                    accountLog($v['user_id'], 0, -$v['pay_points'], '系统执行积分清零');
                }
                adminLog('管理员手动清零积分成功');
            }
            ini_set("max_execution_time",30);
            $this->ajaxReturn(['msg' => '积分清零成功！','status' => 1, 'result' => '']);
        }
    }
}