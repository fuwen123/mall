...
class Report extends Base
{
    ...
    public function expense_log()
    {
        ...
        $count = Db::name('expense_log')->alias('e')->join("__ADMIN__ a", "a.admin_id = e.admin_id")->where($map)->count();
        $page = new Page($count);
        $lists = Db::name('expense_log')->alias('e')->join("__ADMIN__ a", "a.admin_id = e.admin_id")->where($map)->limit($page->firstRow . ',' . $page->listRows)->order('id desc')->select();
    ...
    {
        $goods_name = I('goods_name');
        $selected_goods_ids = I('selected_goods_ids');
        $where = [
            'od.pay_time' => ['Between', "$this->begin,$this->end"],
            'od.order_status' => ['notIN', '3,5'],
            'og.is_send' => 1,
