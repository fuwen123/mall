...
class System extends Base
{
    ...
    public function distribut()
    {
        ...
        $this->assign('inc_type','distribut');//当前配置项
        $this->assign('config',$config);//当前配置项
        return $this->fetch();
    }
    ...
    {
        $menu_list = db('menu_cfg')->where('')->select();
    ...
    public  function addEditNav(){
        ...
        $config = \think\config::get('shop_info');
        $system_nav = array(
    ...
    public function delNavigations(){
        ...
        M('Navigation')->whereIn('id',$navigationsIds)->delete();
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/System/navigationList")]);
    }
...
//		tpCache($param['inc_type'],$param); //注释掉，不注释会出现重复写入数据库
     ...
     */        
     public function login_task()
     {
        ...
        db("Cart")->where("user_id = 0 and  add_time < $time")->delete();            
        $today_time = time();
		
		// 删除 cart表垃圾数据 删除一个月以前的 
		$time = time() - 2592000; 
        db("cart")->where("add_time < $time")->delete();		
		// 删除 tp_sms_log表垃圾数据 删除一个月以前的短信
        db("sms_log")->where("add_time < $time")->delete();				
    ...
    public function delList(){
        ...
        empty($ids) && $this->ajaxReturn(['status' => -1,'msg' =>"非法操作！",'data'  =>'']);
        $listIds = rtrim($ids);
        M('system_menu')->whereIn('id',$listIds)->delete();
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("Admin/System/right_list")]);
    }
	//清除所有活动数据
