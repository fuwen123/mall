...
use app\common\model\saas\WechatOpen;
...
class Wechat extends Base
{
    ...
    public function setting()
    {
        ...
        if (!empty($id)) {
            ...
            $apiurl = SITE_URL . '/index.php?m=Home&c=Weixin&a=index';
...
//        );
    ...
    public function delRelies(){
        $ids = I('post.ids','');
        empty($ids) && $this->ajaxReturn(['status' => -1,'msg' =>"非法操作！",'data'  =>'']);
        $reliesIds = rtrim($ids);
        $logic = new WechatLogic($this->wx_user);
        $return = $logic->deleteAutoReplys($reliesIds);
        $this->ajaxReturn($return);
    }
    ...
    * 第三方平台公众号授权页面
    */
    public function third_authorization(){
        $public = WechatPublicAuthorizer::get(['user_id' => $this->appService['user_id'], 'deleted' => 0]);
        $public['is_auth'] = !empty($public) ? 1 : 0;
     ...
     * 第三方平台授权公众号的开放平台
     */
    public function getWx3rdOpen(){
        if(!$public = WechatPublicAuthorizer::get(['user_id' => $this->appService['user_id'], 'deleted' => 0])){
            $this->error('请先授权公众号');
        }       
        $open = WechatOpen::get(['user_id' => $this->appService['user_id'], 'deleted' => 0]);
    ...
    /**
     * 第三方平台代替公众号创建开放平台账号
     * @param $state 1 = 公众号 ，2 = 小程序
     */
    public function regWx3rdOpen(){
        $state = input('state/d'); //来源，用于区分是操作小程序还是公众号
         
        if($state == 1){
            $public = WechatPublicAuthorizer::get(['user_id' => $this->appService['user_id'], 'deleted' => 0]);
            $reg = new \app\common\logic\saas\WechatPublicLogic($public);
            $rs = $reg->createWx3rdOpen();
            if(!$rs){
                $this->ajaxReturn(['status'=>0,'msg'=>$reg->getError()]);
            }
     ...
     * @param $state 1 = 公众号 ，2 = 小程序
     */
    public function bindWx3rdOpen(){
        $state = input('state/d'); //来源，用于区分是操作小程序还是公众号
        $status = input('status/d',1); //操作状态
   
        $open = WechatOpen::get(['user_id' => $this->appService['user_id'], 'deleted' => 0]);
        $reg = new \app\common\logic\saas\WechatOpen($open);
        if($state == 1){
            $public = WechatPublicAuthorizer::get(['user_id' => $this->appService['user_id'], 'deleted' => 0]);                                
            $reg->setWechatPublic($public);
            $rs = $reg->bind(1,$status);      
        }else{
            $miniapp = Miniapp::get(['user_id' => $this->appService['user_id'], 'is_auth' => 1]);   
            if(!$miniapp){
                $this->ajaxReturn(['status'=>0,'msg'=>'小程序暂未授权绑定']);
            }           
            $reg->setMiniApp($miniapp);
            $rs = $reg->bind(2,$status);
        }
        if( !$rs){
            $this->ajaxReturn(['status'=>0,'msg'=> $reg->getError()]);
        }
       
        $this->ajaxReturn(['status' => 1,'msg' =>  $status == 1 ? '绑定成功' : '解绑成功' ]);
    }   
}