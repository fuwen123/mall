...
//            $this->error('此订单，已完成支付!');
    ...
    public function buy_upgrade(){
        ...
        $user = session('user');
        $level_id = I('level_id');
        $order = db('recharge')->where(array('level_id'=>$level_id,'user_id'=>$user['user_id']))->find();
        if(empty($order)){
            $level_info = db('distribut_level')->where('level_id',$level_id)->find();
            $data['user_id'] = $user['user_id'];
            $data['nickname'] = $user['nickname'];
            $data['order_sn'] = 'recharge'.get_rand_str(10,0,1);
            $data['ctime'] = time();
        ...
        if (0 == $order['pay_status']) {
            ...
            $payment_arr = M('Plugin')->where("`type` = 'payment'")->getField("code,name");
            db('recharge')->where("order_id", $order_id)->save(array('pay_code' => $this->pay_code, 'pay_name' => $payment_arr[$this->pay_code]));
            //微信JS支付
            if ($this->pay_code == 'weixin' && $_SESSION['openid'] && strstr($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger')) {
                $code_str = $this->payment->getJSAPI($order);
                exit($code_str);
