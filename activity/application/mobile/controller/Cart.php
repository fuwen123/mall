...
use app\common\model\BespeakTemplate;
...
class Cart extends MobileBase {
    ...
    public function index()
    {
        ...
        $hot_goods = db('Goods')->where('is_hot=1')->limit(20)->order('goods_id DESC')->cache(true, TPSHOP_CACHE_TIME)->select();
    ...
    public function bespeakUpImg(){
        // 晒图片
        $files = request()->file('bespeak_img_file');
        $save_url = UPLOAD_PATH.'bespeak/' . date('Y', time()) . '/' . date('m-d', time());
        if($files) {
        ...
        if (!empty($comment_img)) {
            $this->ajaxReturn(['status' => 1,'msg' =>'上传成功','data' =>implode(',',$comment_img)]);
        }
    ...
    public function cart2(){
        ...
        $is_use_integral = Db::name("config")->where("name='is_use_integral'")->getField('value');//积分使用情况
        $this->assign('is_use_integral', $is_use_integral);  //优惠券，用able判断是否可用
        ...
        //获取getSignPackage
        if(is_weixin()){
            $wechat_logic = new WechatUtil();
            $sign_package = $wechat_logic->getSignPackage();
    ...
    public function cart3(){
        ...
        $data = input('request.');
        $bespeakForm = input('bespeak_form/a');
        ...
        try {
		    ...
		    ->setBespeakForm($bespeakForm)->addNormalOrder();
                $cartLogic->clear();
    ...
    public function cart4(){
        ...
        if($order['order_status'] == 3){
            if($is_virtual==2){
                $this->error('该订单已取消',U("Mobile/Order/bespeak_detail",array('id'=>$order['order_id'])));
            }else{
        ...
        if($order['pay_status'] == 1){
            if($is_virtual==2){
                $order_detail_url = U("Mobile/Cart/bespeak_success",array('goods_name'=>input('goods_name'),'take_time'=>date('Y-m-d H:i:s',input('take_time'))));
            }else{
                $order_detail_url = U('mobile/order/paySuccess',['id'=>$order['order_id']]);
            }
        ...
        if($is_virtual==2){
            ...
            try {
                ...
                $error = $t->getErrorArr();
                $this->error($error['msg'],U("Mobile/Order/bespeak_detail",array('id'=>$order['order_id'])));
            }
        }
        ...
        if ($order['prom_type'] == 8) {
            //如果是砍价订单，支付尾款
            $promotion_bargain = db('promotion_bargain')->where(['id'=>$order['prom_id']])->find();
            if ($promotion_bargain['end_time'] < time() ) {
                $this->error('该活动已结束，不能支付');
            }
            $promotion_bargain_goods_item = db('promotion_bargain_goods_item')->where(['bargain_id'=>$order['prom_id'],'item_id'=>$order['order_goods'][0]['item_id']])->find();
    ...
    public function integral(){
        ...
        if (!empty($goods['price_ladder'])) {
            ...
            $price_ladder = $goods['price_ladder'];
            $goods['shop_price'] = $goodsLogic->getGoodsPriceByLadder($goods_num, $goods['shop_price'], $price_ladder);
        }
        $goods['shop_price'] = $goods['shop_price'] - ($goods['exchange_integral'] / 10);
     ...
     * 预约成功模板
     */
    public function bespeak_success(){
        return $this->fetch();
    }

    public function shop(){
        $shop_id = I('shop_id/d',false);
        if(!$shop_id){
            $this->error('门店不存在',U('mobile/index/index'));
        }
