...
//            'prom_type'=>['lt',5],//虚拟拼团订单不列出来
            'prom_type'=>['in',[0,1,2,3,4]],//虚拟拼团订单不列出来
            'shop_id'=>0,//过滤预约
        ];
        if($is_shop){
...
//            $this->ajaxReturn(['status'=>1,'msg'=>'获取成功','result'=>$order_list]);
    ...
    public function order_detail()
    {
		...
		if ($childOrder) {
			$this->assign('childOrder', $childOrder);
		}
        //获取订单
    ...
    public function bespeak_list()
    {
        ...
        $order_list = $order->where($where_arr)
            ...
            ->limit($Page->firstRow . ',' . $Page->listRows)->order("order_id DESC")->select();
        $this->assign('bespeak_list', $order_list);
        if ($_GET['is_ajax']) {
            return $this->fetch('ajax_bespeak_list');
        }
    ...
    */
    ...
    public function checkReturnInfo()
    {
        ...
        $data['status'] = 2;
        $res = db('return_goods')->where(['id'=>$data['id'],'user_id'=>$this->user_id])->save($data);
        if($res !== false){
    ...
    public function return_goods_refund()
    {
        ...
        $page = new Page($count,10);
        $list = db('return_goods')->where($where)->order("id desc")->limit($page->firstRow, $page->listRows)->select();
        $goods_id_arr = get_arr_column($list, 'goods_id');
        if(!empty($goods_id_arr))
            $goodsList = db('goods')->where("goods_id in (".  implode(',',$goods_id_arr).")")->getField('goods_id,goods_name');
    ...
    public function return_goods_cancel(){
        ...
        $return_goods = db('return_goods')->where(array('id'=>$id,'user_id'=>$this->user_id))->find();
        if(empty($return_goods)) $this->ajaxReturn(['status'=>-1,'msg'=>'参数错误']);
        $res= db('return_goods')->where(array('id'=>$id))->save(array('status'=>-2,'canceltime'=>time()));
        if ($res !== false){
    ...
    public function receiveConfirm(){
        ...
        $return_info=db('return_goods')->field('order_id,order_sn,goods_id,spec_key')->where('id',$return_id)->find(); //查找退换货商品信息
        $update = db('return_goods')->where('id',$return_id)->save(['status'=>3]);  //要更新状态为已完成
        if($update) {
            db('order_goods')->where(array(
    ...
    public function add_comment()
    {
        ...
        $rec_id = I('rec_id/d');
        $order_goods = db('order_goods')->where("rec_id", $rec_id)->find();
    ...
    public function ajaxZan()
    {
        ...
        if (in_array($user_id, $comment_user_id_array)) {  //判断用户有没点赞过
            ...
            db('comment')->where(array('comment_id' => $comment_id))->save($comment_data);
