...
class StockLogic
{
	private $suppliersId;  //供应商id
	private $goodsIds;  //供应商的商品id列表，字符串
    ...
    {
		$goodsIds = Db::name('goods')->where('suppliers_id', $this->suppliersId)->where('audit', 0)->getField('goods_id,goods_id');
		if ($goodsIds) {
     ...
     * 库存盘点（快速修改库存）
     */
    public function doChangeStockVal()
    {
        $ids = I('ids');
        ...
        try{
            ...
            if($item_ids){ //对规格商品进行库存修改
                ...
                $spec_info = $spec_model->alias('a')
                    ->where('a.item_id','in',$item_ids)->where('a.goods_id', 'in', $this->goodsIds)->join('Goods g', 'a.goods_id=g.goods_id')
                    ->field('a.goods_id, a.store_count, a.key_name, g.goods_name')
                    ->select();  //获取规格商品信息
                $stock_log1 = array_map(function($val) use ($num, $ctime){  //生成规格商品的stock_log记录
                    $val['stock'] = $num - $val['store_count']; //库存修改量
                    $val['muid'] = $this->suppliersId;
                ...
                $spec_model->where('item_id','in', $item_ids)->where('goods_id', 'in', $this->goodsIds)->update(['store_count'=>$num]);  //更新spec_goods_price的store_count
                $goods_id = $spec_model->where('item_id','in',$item_ids)->where('goods_id', 'in', $this->goodsIds)->field('goods_id')->select(); //找出规格商品对应的goods_id
            ...
            if($goods_ids){ //对无规格商品进行库存修改
				$temp_goods_ids = explode(',', $this->goodsIds);
				$goods_ids = array_intersect($goods_ids, $temp_goods_ids);  //过滤掉不是此供应商下商品的id
                $goods_info = $goods_model->where('goods_id','in', implode(',',$goods_ids))->field('goods_id, store_count, goods_name')->select();//获取原先库存量
                $stock_log2 = array_map(function($val) use ($num, $ctime){  //生成无规格商品的stock_log记录
                    $val['stock'] = $num - $val['store_count']; //库存修改量
                    $val['muid'] = $this->suppliersId;
                    $val['change_type'] = 3;  //操作类型：3库存盘点
                    $val['ctime'] = $ctime;
    ...
    private function get_map(){
		...
		$map['a.goods_id'] = ['in', $this->goodsIds];
        return $map;
    }
