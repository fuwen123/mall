...
class GoodsLogic extends Model
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
       ...
       foreach ($clo_name as $k => $v) 
       {
        ...
        if(count($spec_arr2) > 0){
            $str_fill .='<td><input id="item_cost_price" value="0" onkeyup="this.value=this.value.replace(/[^\d.]/g,&quot;&quot;)" onpaste="this.value=this.value.replace(/[^\d.]/g,&quot;&quot;)"></td>
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
			...
			$keySpecGoodsPrice[$item_key][price] ? false : $keySpecGoodsPrice[$item_key][price] = 0; // 价格默认为0（供应商不写）
			...
			$keySpecGoodsPrice[$item_key][commission] ? false : $keySpecGoodsPrice[$item_key][commission] = 0; //佣金默认为0（供应商不写）
            ...
            $str .="<td><button type='button' class='btn btn-no btn-default delete_item'>无效</button></td>";
            $str .="</tr>";
       }
        $str .= "</table>";
       return $str;   
    ...
    public function afterSave($goods_id)
    {
        ...
        // 商品货号
        $goods_sn = $goods_sn_pre.str_pad($goods_id,7,"0",STR_PAD_LEFT);
        ...
        if ($goods_item) {
            ...
            foreach ($goods_item as $k => $v) {
                ...
                update_stock_log($supplier['suppliers_id'],$stock,array('goods_id'=>$goods_id,'goods_name'=>I('goods_name'),'spec_key_name'=>$v['key_name']), '', 1);
                // 修改商品后购物车的商品价格也修改一下
            ...
            if($keyArr){
                // 无活动的规格可删除
                Db::name('spec_goods_price')->where('goods_id',$goods_id)->where('prom_type',0)->whereNotIn('key',$keyArr)->delete();
            }
        }else{
