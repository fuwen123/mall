...
class Shop extends Model
{
    ...
    {
        $province = Db::name('region')->where(['id' => $data['province_id']])->value('name');
        $city = Db::name('region')->where(['id' => $data['city_id']])->value('name');
        $district = Db::name('region')->where(['id' => $data['district_id']])->value('name');
        $address = $province . ' ' . $city . ' ' . $district . ' ' . $data['shop_address'];
        return $address;
    }
    /**
     * 设置添加时间
