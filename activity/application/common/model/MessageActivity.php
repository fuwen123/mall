...
class MessageActivity extends Model
{
    ...
    public function getGoodsIdAttr($value, $data)
    {
        ...
        switch ($data['mmt_code']) {
            ...
            case 'team_activity':
                ...
                break;
            case 'promotion_bargain_activity':
                // 砍价
                $return_arr = Db::name('promotion_bargain')->where('id', $data['prom_id'])->find();
                break;
            default:
                $return_arr = ['goods_id'=>0, 'item_id'=>0];
                break;
    ...
    public function getMobileUrlAttr($value, $data)
    {
        if ($data['mmt_code'] == 'flash_sale_activity') {
            ...
            $flash_sale = Db::name('team_activity')->where('team_id', $data['prom_id'])->find();
            $uri = U("Mobile/Goods/goodsInfo", ['id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
        }  elseif ($data['mmt_code'] == 'promotion_bargain_activity'){
            // 砍价
            $goods_id = Db::name('promotion_bargain')->alias('p')->join('__PROMOTION_BARGAIN_GOODS_ITEM__ g','p.id = g.bargain_id')->where(['id' => $data['prom_id']])->field('g.goods_id,item_id')->find();
            $uri = U("Mobile/Goods/goodsInfo", ['id' => $goods_id['goods_id'],'item_id'=>$goods_id['item_id']]);
        } else {
            $uri = '';
        }
        return $uri;
