...
class CartLogic extends Model
{
    ...
    public function setGoodsModel($goods_id)
    {
        if ($goods_id > 0) {
            ...
            //加了判断，上架才能买
            $this->goods = $goodsModel::get(['goods_id'=>$goods_id,'is_on_sale'=>1]);
        }
    }
    ...
    public function buyNow()
    {
        ...
        //$this->goods['prom_type'] 加了个godos判断，避免多规格的商品prom_type没有活动，规格prom_type却有活动，导致报错问题
        if ($goodsPromFactory->checkPromType($prom_type) && $goodsPromFactory->checkPromType($this->goods['prom_type'])) {
            $goodsPromLogic = $goodsPromFactory->makeModule($this->goods, $this->specGoodsPrice);
            if(!$goodsPromLogic){
                throw new TpshopException('立即购买', 0, ['status' => 0, 'msg' => '商品活动不存在', 'result' => '']);
            }
    ...
    private function addNormalCart()
    {
        ...
        if ($userCartGoods) {
            ...
            //如果该商品没有存在购物车
            if ($this->goodsBuyNum > $store_count) {
                throw new TpshopException("加入购物车", 0, ['status' => 0, 'msg' => '商品库存不足，剩余' . $store_count]);
            }
            //如果有阶梯价格,就是用阶梯价格
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
...
//        halt(($promGoodsLogic->checkActivityIsEnd() || !$promGoodsLogic->checkActivityIsAble()));
    ...
    public function getCartNum(){
        ...
        $n = Db::name('cart')->where($cartWhere)->count();
        return $n;
    }
    ...
    public function getCouponCartList($cartList, $userCouponList)
    {
        ...
        foreach ($userCouponArray as $couponKey => $couponItem) {
            if ($userCouponArray[$couponKey]['coupon']['use_type'] == 0) { //全店使用优惠券
                ...
                foreach ($cartList['cartList'] as $tKey => $Item) {
                    //追溯上级分类，优惠券
                    $all_cat = db('goods_category')->where(['id'=>['in',$Item['goods']['cat_id']]])->field('parent_id_path')->select();
                    $arr = [];
                    foreach ($all_cat as $k=>$v)
                    {
                        $arr = array_merge($arr,explode('_',$v['parent_id_path']));
                    }
                    $couponGoodsCatId = $arr;
