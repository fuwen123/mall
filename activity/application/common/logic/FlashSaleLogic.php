...
class FlashSaleLogic extends Prom
{
     ...
     * 没取消，没作废的订单商品总数量，
     */
    public function getActivityGoodsNum(){
        $where['order.deleted'] = 0;
        $where['order.order_status'] = ['in','0,1,2,4'];
        $where['order_goods.prom_id'] = $this->flashSale['id'];
        $where['order_goods.prom_type'] = 1;
        $list = Db::view('order_goods','goods_id,goods_num,is_send,prom_type,prom_id')
            ->view('order','order_status,deleted','order.order_id=order_goods.order_id','LEFT')
            ->where($where)
            ->select();
        if($list){
            $goods_num = array_sum(array_map(function($val){return $val['goods_num'];}, $list));
        }else{
            $goods_num = 0;
        }
        return $goods_num;
    ...
    public function buyNow($buyGoods){
        // 先下单的，先锁库存，后下单不能抢，等前面下单取消了，才可以再次下单，其它活动也应该适用
        if($this->getActivityGoodsNum() >= $this->flashSale['goods_num']){
            throw new TpshopException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '该抢购商品库存已锁定！', 'result' => '']);
        }
