...
class MessageNoticeLogic extends MessageBase
{
    ...
    public function getCodeByType($type)
    {
        switch ($type) {
            ...
            case 6:
                ...
                break;
			case 7:
                $mmt_code = 'supplier_withdrawals_notice';
                break;
            default:
                $mmt_code = '';
                break;
     ...
     * @param $money
     */
    public function supplierWithdrawalsNotice($supplier_withdrawals_id, $uid, $money, $suppliers_name)
    {
        $this->setType(7);
        $this->setMessageTitle('供应商提现已到账');
        $this->setMessageContent(''); // 空着，使用模板内容
        $this->setPromId($supplier_withdrawals_id);
        $this->setUsers([$uid]);
        $this->setMessageVal(['money' => $money, 'suppliers_name' => $suppliers_name]);
        $this->sendMessage();
    }
