...
class ActivityLogic extends Model
{
    ...
    public function getCouponList($atype, $user_id, $p = 1)
    {
        ...
        $coupon_list = (new Coupon())->alias('c')
            ->field("*,send_end_time-'$time' as spacing_time,gc.goods_id,gc.goods_category_id")
...
//        }
        ...
        $coupon_list = $query->group('c.id')->select();
        if (!(is_array($coupon_list) && count($coupon_list) > 0)) {
            return [];
        }
        ...
        foreach ($coupon_list as $k => $coupon) {
            ...
            switch ($coupon['use_type']){
                case 1;
                    ...
                    break;
                case 2;
                    // 如果 有商品id,查分类是不是这优惠券的指定分类,否则按原先处理
                    if($goods_id > 0){
                        $cat_id = Db::name('goods')->where('goods_id',$goods_id)->cache(true)->value('cat_id');
                        // 不是这个商品的分类，就删除
                        if($cat_id != $coupon['goods_category_id']){
                            unset($coupon_list[$k]);
                        }
                    }else{
