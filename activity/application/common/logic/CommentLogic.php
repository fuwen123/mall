...
class CommentLogic extends Model
{
	...
	public function getCommentInfo($comment_id)
	{
		...
		$comment_info = M('comment')->field('*,ceil((deliver_rank + goods_rank + service_rank) / 3) as goods_rank')->where(array('comment_id'=>$comment_id))->find();
    ...
    public function addGoodsComment($add)
    {
        ...
        //更新订单商品表状态
        if($add['item_id'] > 0){
            M('order_goods')->where(['goods_id'=>$add['goods_id'],'order_id'=>$add['order_id'],'item_id'=>$add['item_id']])->save(['is_comment'=>1]);
        }else{
