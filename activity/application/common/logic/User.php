...
class User
{
    ...
    public function checkOauthBind()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false && ACTION_NAME != 'bind_account') {
            ...
            if(cache(I('unique_id'))){
                session('third_oauth',cache(I('unique_id')));
            }
        }
        ...
        if ($unionid) {
            ...
            $thirdUser = Db::name('oauth_users')->where(['user_id' => $this->user['user_id'], 'oauth' => $oauth])->find();
            if ($thirdUser) {
                throw new TpshopException('关联账号', 0, ['status' => 0, 'msg' => '此账号已绑定其它' . $oauthCN . '账号']);
            }
        }
    }
    ...
    /**
     * 如果账号，已有绑过分销，这里增加下线，就多余
     * @throws \think\Exception
     */
    public function doLeader()
    {
        /* $first_leader = cookie('first_leader')?: cache(I('unique_id').'first_leader');  //推荐人id
        cache(I('unique_id').'first_leader',null);
        if ($first_leader && $first_leader != $this->user->first_leader) {
