...
class Team
{
    ...
    public function doOrderPayAfter(){
        ...
        if ($teamFound) {
            ...
            if($teamFollow){
                ...
                //如果参团人数满足成团条件
                if($teamFollow->team_found->join >= $teamFollow->team_found->need){ // 并且不是抽奖团
    ...
    public function buy()
    {
        if (empty($this->teamActivity) || $this->teamActivity['status'] != 1) {
            throw new TpshopException('拼团购买商品',0,['status' => 0, 'msg' => '该商品拼团活动不存在或者已下架，不接受新的订单', 'result' => '']);
        }
        ...
        if (empty($goods) || $goods['is_on_sale'] != 1) {
            throw new TpshopException('拼团购买商品',0,['status' => 0, 'msg' => '该拼团活动的商品不存在或者已下架，无法下单', 'result' => '']);
        }
