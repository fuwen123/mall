...
class WechatLogic
{
    ...
    public function sendTemplateMsgOnTeam($found_id){
        ...
        $follow_user_ids = Db::name('team_follow')->where('found_id',$found_id)->select();
    ...
    public function sendTemplateMsgOnPaySuccess($order)
    {
		...
		$oldOrder = Db::name('order')->where('order_id', $order['order_id'])->find();
		if ($oldOrder && $oldOrder['deleted'] == 1) {
			//如果此订单已被假删除，表示此订单在付款后被拆单，修改发送的个别信息
			$newOrder = Db::name('order')->where('parent_sn', $oldOrder['order_id'])->select();
			$order['order_id'] = $newOrder[0]['order_id'];
			$orderSn = array_column($newOrder, 'order_sn');
    ...
    public function sendTemplateMsgOnDeliver($deliver)
    {
        ...
        $data = [
            ...
            'keyword2' => ['value' => $deliver['shipping_name'] ?: '无'] ,
            'keyword3' => ['value' => $deliver['delivery_sn'] ?: '无'],
     ...
     * 批量删除自动回复
     */
    public function deleteAutoReplys($reliesIds)
    {


        WxKeyword::whereIn('pid',$reliesIds)->delete();

        WxReply::whereIn('id',$reliesIds)->delete();

        return ['status' => 1,'msg' => '操作成功','url'=>U("Admin/Wechat/auto_reply")];
    }
}