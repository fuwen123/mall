	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
		...
		if($order['coupon_price'] >0){
			...
			if (empty($order['parent_sn'])) {
				...
				if ($parentOrderId) {
					M('coupon_list')->where(array('order_id'=>$parentOrderId,'uid'=>$user_id))->save($res);
				}
			}
		}
...
//			$data['action_note'] = $user_note;
    ...
    public function alterReturnGoodsInventory($order, $rec_id='')
	{
        ...
        if($order['prom_type'] == 8){
            //取消订单恢复砍价活动的库存
            db('promotion_bargain_goods_item')->where(['bargain_id'=>$order['prom_id'],'item_id'=>$order_goods[0]['item_id']])->setDec('buy_num',$order_goods[0]['goods_num']);
        }
		foreach($order_goods as $key=>$val){
			...
			if ($val['prom_type'] == 1 || $val['prom_type'] == 2) {
				...
				if ($prom['is_end'] == 1 && $prom['end_time'] > time()) {
					M($tb)->where("id", $val['prom_id'])->update(['is_end'=>0]);
					M('goods')->where('goods_id',$prom['goods_id'])->update(['prom_type'=>$val['prom_type'],'prom_id'=>$val['prom_id']]);
				}
			}
		}
	}
...
//            $this->error('excel数据格式错误,请下载并参照excel模板',U('Seller/Order/delivery_excel'));
		...
		foreach ($arr as $k => $v) {
			...
			$order_id = \app\common\model\Order::get(['order_sn'=>$v[0]]);
