...
class UsersLogic extends Model
{
    ...
    */
    ...
    public function reg($username,$password,$password2,$push_id = 0,$invite=array(),$nickname="",$head_pic=""){
        ...
        $map['reg_time'] = time();
        $map['first_leader'] = cookie('first_leader')?: cache(I('unique_id').'first_leader');  //推荐人id
...
//        $user = M('users')->where('user_id', $user_id)->find();
    ...
    public function add_comment($add){
        ...
        if($row)
        {
            //更新订单商品表状态
            if($add['item_id'] > 0){
                M('order_goods')->where(array('rec_id'=>$add['rec_id'],'order_id'=>$add['order_id'],'item_id'=>$add['item_id']))->save(array('is_comment'=>1));
            }else{
