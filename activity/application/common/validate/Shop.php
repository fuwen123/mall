...
class Shop extends Validate
{
    ...
    protected $rule = [
        ...
        'monday'=> 'checkShopDays'
    ];
    protected $scene = [
        'edit' => ['shop_id', 'shop_name', 'province_id', 'city_id', 'district_id', 'shop_address', 'shop_phone', 'shop_images[]','longitude','latitude','monday'],
        'add' => ['shop_name', 'user_name', 'password', 'shopper_name', 'province_id', 'city_id', 'district_id', 'shop_address', 'shop_phone', 'shop_images[]','longitude','latitude','monday'],
    ];
    //错误信息
    protected $message = [
        ...
        'monday.checkShopDays'=>'请至少选择一天营业日'
    ];
     ...
     * @return bool|string
     */
    protected  function checkShopDays($value, $rule, $data){
        $days = ['monday','tuesday','wednesday','thursday','friday','saturday','sunday'];
        $temp = 0;
        foreach ($days as $v){
            if(!in_array($data[$v],['0','1'])){
                return false;
            }
            $temp += $data[$v];
        }
