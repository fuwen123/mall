...
class Seller extends MobileBase
{
    ...
    */
    ...
    public function logout()
    {
        session_unset();
        session_destroy();
        db('users')->where(["user_id" => cookie('user_id')])->save(['token' => '']);
...
//    	return $this->fetch();
    ...
    public function do_login()
    {
        ...
        if ($res['status'] == 1) {
            //获取最新余额
            $UsersLogic = new \app\common\logic\UsersLogic();
            $UsersLogic->checkUserWithdrawals($res['result']);
            ...
            db('users')->where(["user_id"=>$res['result']['user_id']])->save(['token' => $token, 'last_login' => time()]);
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            ...
            if (!$userLogic->update_info($this->user_id, $post))
                $this->error("保存失败",U('User/userinfo'));
    ...
    */
    ...
    public function cancel_collect()
    {
        ...
        if (M('goods_collect')->where(['collect_id' => $collect_id, 'user_id' => $user_id])->delete()) {
            $this->success("已取消收藏", U('User/collect_list'));
        } else {
            $this->error("未取消收藏", U('User/collect_list'));
        }
    }
...
//        }
    ...
    public function poster_qrcode()
    {
        ...
        $project_path = '/public/upload/poster/'.I('_saas_app','all');  // 原是 /public/images，linux可能无权限
