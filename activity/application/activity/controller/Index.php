...
//        $hot_goods = D('goods')->where("is_hot=1 and is_on_sale=1")->order('goods_id DESC')->limit(6)->cache(true,TPSHOP_CACHE_TIME)->select();//首页热卖商品
//        $this->assign('hot_goods',$hot_goods);
        $recommend_goods = D('goods')->where("is_recommend=1 and is_on_sale=1")->order('sort DESC')->limit(6)->cache(true,TPSHOP_CACHE_TIME)->select();//首页推荐商品

        //获取拼团列表
        $team_activity = D('team_activity')->with('teamGoodsItem')->where(['status'=>1,'deleted'=>0])->limit(6)->cache(true,TPSHOP_CACHE_TIME)->select();
