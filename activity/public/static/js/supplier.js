...
//使用title内容作为tooltip提示文字
    ...
    if ($.cookie('workspaceParamSupplier') == null) {
        // 默认选择第一个菜单
        //$('.nc-module-menu').find('li:first > a').click();
        openItem('index|Goods');
    } else {
        openItem($.cookie('workspaceParamSupplier'));
    }
    // 导航菜单  显示
...
function openItem(param) {	
    ...
    $('#workspace').attr('src','/index.php?m=Supplier&c=' + data_str[1] + '&a=' + data_str[0]);
    $.cookie('workspaceParamSupplier', data_str[0] + '|' + data_str[1], { expires: 1 ,path:"/"});
}
