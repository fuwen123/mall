...
/***********************************************************************************************************/
    ...
    function img_call_back(fileurl_tmp){
        ...
        if(choice_type=='4'){
            ...
            if($('.tpd-edits-hidden.tpd-editing .tpdm-wstyle'+item).find('img').length>0){
                $('.tpd-edits-hidden.tpd-editing .tpdm-wstyle'+item).find('img')
            }else{
                $('.tpd-edits-hidden.tpd-editing .tpdm-wstyle'+item).html("<img src='"+fileurl_tmp+"'>")
            }
            //把编辑内容按顺序存入block数组
        ...
        if(choice_type == '14'){
            ...
            // console.log(video_url,fileurl_tmp)
...
function createModule(eidtid){
    ...
    */
    ...
    if(eidtid=='2'){    //生成快捷入口板块时默认样式选择为方形  导航块赋初始值
        ...
        block[divid]['diy_bg_color']="#ffffff"
        block[divid]['nav']=[
            {
    ...
    if(eidtid=='3'){
        ...
        block[divid].num=4;
        block[divid].name_show=0;   //列表名称显示  0不显示  1显示
        block[divid].order=0;       //商品排序
        block[divid].list_bg_color = "#ffffff"
        // $('#goods-name').find('input').eq(0).prop('checked',true)
        block[divid]['nav']=[
        ...
        // $('#goods-color').find('input[type=color]').val("#ffffff")
        show_tool_list()
    }
    ...
    if(eidtid=='6'){    //生成营销活动板块时默认营销活动赋值
        ...
        block[divid].activity_bg_color = '#ffffff'
        //拼团类型时才有以下3个属性值
        block[divid].tab_type=0;            //活动标题是否显示 0不使用  1使用
    ...
    if(eidtid=='7'){    //生成优惠券板块时默认显示风格赋值
        ...
        block[divid]['nav']=[];
        block[divid]['favourable_bg_color'] = '#ffffff'
        block[divid]['type_index'] = '0'
        $('.tpd-coupon-list .tpd-addlink-target .coupon_span').text(3)
        // $('#coupon-style1').find('label').find('input').eq(0)[0].checked = true
        show_favourable()
    }
    ...
    if(eidtid=='13'){
        ...
        //拖拽时就按默认条件读取商品
        $('#goodIds-bgcolor input[type=color]').val('#ffffff')
        $.post("/index.php/Admin/Block/goods_list_block",{'num':block[divid].num,'order':block[divid].order,'block_type':13},function(res){
    ...
    if(eidtid=='15'){   //生成图文板块时默认风格赋值
        ...
        block[divid]['title']='';
        block[divid]['content']='';
        // block[divid]['news_bg_color'] = '#ffffff'
        block[divid]['nav']=[
            {
                "pic":""
            },
    ...
    if(eidtid=='20'){
        block[divid].blank_bg_color = '#ffffff'
        block[divid].blank_range = '20'
        // $('.tpd-blank .tpd-txtnav-text input[type=color]').val("#ffffff")
        // $('.tpd-blank .tpd-txtnav-text input[type=range]').val("20")
        // $('.tpd-blank .tpd-txtnav-text em').text("20px")
        show_blank()
    }
    //新闻
    if(eidtid=='12'){   //生成新闻板块
        ...
        $('#goods-num-bgcolor input[type=color]').val("#ffffff")
            ...
            switch(choice_type){
                ...
                case '0':
                    ...
                    if(block[divid]['url']){
                        ...
                        $('.tpd-diy-js').eq(0).find('.tpd-addlink-target').eq(0).html('添加链接目标');
                    }
                    break;
                ...
                case '3':
                    ...
                    //     }
                    // })
                    // if($('#goods-name').find('input').eq(0).val()==block[divid].name_show){
                    //     $('#goods-name').find('input').eq(0).prop('checked',true)
                    // }else {
                    //     $('#goods-name').find('input').eq(0).prop('checked',false)
                    // }
                ...
                case '7':
                    ...
                    //     $('#coupon-style1').find('input').eq(0).attr('checked','checked').prop('checked','checked');
                    //     $('#coupon-style1').find('input').eq(1).removeAttr('checked');
                    // }else{
                    //     $('#coupon-style1').find('input').eq(1).attr('checked','checked').prop('checked','checked');
                    //     $('#coupon-style1').find('input').eq(0).removeAttr('checked');
                    // }
                    show_favourable()
                    break;
...
//  console.log(block[divid].nav);
    for(var i=0;i<block[divid].nav.length;i++){
        ...
        li_str+='</div><div class="tpd-addlink-target '+addlink+'"><i></i>'+'链接'+'</div></div></div> <div class="c-suggest"><p>建议尺寸宽度80px，高度80px</p></div>';
        li_str+='<div class="tpd-cdiy-identity"></div>'
...
function footer_split(){
    console.log(divid,block[divid])
    if(typeof(block[divid]) == 'undefined' || typeof(block[divid].nav) == 'undefined'){
        return;
    }
...
function goods_list_edit(){
    ...
    for(var i=0;i<block[divid].nav.length;i++){
        ...
        str+='</dl></div>';
        str+='<div class="tpd-cdiy-identity"></div>';
        str+='<a class="tpd-del-nav" href="javascript:;"></a></li>';
    }
    $('#tab_list').html(str);
}
...
function show_goods_hand1(){
    $("#pop-link2").find('input').remove('checked').prop('checked',false)
