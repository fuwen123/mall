...
class Pointcart extends MobileMember {
    ...
    public function step1() {
        ...
        //实例化收货地址模型（不显示自提点地址）
        $address_list = model('address')->getAddressList(array('member_id' => $this->member_info['member_id'], 'dlyp_id' => 0), 'address_is_default desc,address_id desc');

        //收货地址为空 返回兑换里面信息
        if ($address_list){
            ds_json_encode(10000, '', array('pointprod_arr' => $data['data'], 'address_info' => $address_list[0]));
        }else{
            ds_json_encode(10001,lang('information_consignee2'),array('pointprod_arr' => $data['data']));
        }
    }
