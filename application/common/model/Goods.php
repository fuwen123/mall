...
class Goods extends Model {
     ...
     * @return type
     */
    public function getGoodsUnionList($condition, $field, $order = 'goodscommon.mall_goods_commend desc,goodscommon.mall_goods_sort asc',$group='', $page = 0,$limit=0) {
        ...
        if ($page) {
            $result=$result->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $result;
            return $result->items();
        } else {
    ...
    public function getGoodsListForPromotion($condition, $field = '*', $page = 0, $type = '') {
        switch ($type) {
            ...
            case 'bundling':
                ...
                $condition['goodscommon.goods_state'] = self::STATE1;
                $condition['goodscommon.goods_verify'] = self::VERIFY1;
                return $this->getGoodsUnionList($condition, $field, 'goodscommon.goods_commonid asc','', $page);
            case 'combo':
                $condition['is_virtual'] = 0;
                $condition['is_goodsfcode'] = 0;
                $condition['is_presell'] = 0;
