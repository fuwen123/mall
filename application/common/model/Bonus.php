...
class Bonus extends Model {
     ...
     */
    public function getBonusList($condition,$page ,$limit = '',$order='bonus_id desc') {
        if($page){
            $result = db('bonus')->where($condition)->order($order)->paginate($page,false,['query' => request()->param()]);
     ...
     * @return array 数组类型的返回结果
     */
    public function getBonusreceiveList($condition,$page ,$limit='') {
        if($page){
            $result = db('bonusreceive')->where($condition)->paginate($page,false,['query' => request()->param()]);
            $this->page_info=$result;
            return $result->items();
        }else{
