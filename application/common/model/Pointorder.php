...
class Pointorder extends Model {
     ...
     * @return array
     */
    public function getPointorderAndGoodsList($where, $field = '*',  $order = '',$page='') {
        if($page){
            $order_list = db('pointsordergoods')->alias('pointsordergoods')->field($field)->join('__POINTSORDER__ pointsorder','pointsordergoods.pointog_orderid=pointsorder.point_orderid')->where($where)->order($order)->paginate($page,false,['query' => request()->param()]);
            $this->page_info = $order_list;
            $order_list = $order_list->items();
            return $order_list;
        }else{
