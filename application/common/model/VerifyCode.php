...
class VerifyCode extends Model {
     ...
     * @return type
     */
    public function getVerifyCodeList($condition, $page = '', $order = 'verify_code_id desc') {
        if ($page) {
            $result = db('VerifyCode')->where($condition)->order($order)->paginate($page, false, ['query' => request()->param()]);
